...
//    model->removeRow(row);//移除
    QString s = this->MakeMsg("my/hhh",TEXT);
    qDebug()<<this->MakeMsg("1.1.1.1/my",TEXT);
    this->ResolveMsg(s);
}
...
int MultiChatDlg::ResolveMsg(QString str)
{
    ...
    //lenIp = bytes[2];
    //ip = bytes.mid(3,lenIp);

   // lenName = bytes[3+lenIp];
  //  name = bytes.mid(4+lenIp,lenName);
    switch((int)type)
    {
        case ONLINE:
//        len = bytes[2];
...
//        len = bytes[10];
//        qDebug()<<len;
//        qDebug()<<bytes.mid(11,len);
//        name = bytes.mid(11,len);
        ...
        qDebug()<<name;
        msg = bytes[4+lenName,(bytes.length()-(3+lenName))];
        if(name == this->name){
